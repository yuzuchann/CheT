...
class CheTModel(nn.Module):
    ...
    def forward(self, tokens, segments, valid_lens=None):
        ...
        bgp_label = self.bgp(self.hidden(encoded_X[:, 0, :]))
        sp_label = self.sp(self.hidden(encoded_X[:, 0, :]))
        
        return sgp_label, bgp_label, sp_label
    

def load_data(df, test_size,batch_size,seed=0):
...
def _get_batch_loss_bert(net,element_ids,proportions,valid_lens,spacegroup,bandgaps,stablility):
    # 前向传播
    sgp_label, bgp_label, sp_label = net(element_ids,proportions,valid_lens=valid_lens.reshape(-1))
    ...
    bgp_label = bgp_label.squeeze(1)
    sp_label = sp_label.squeeze(1)
    
    sgp_l = loss1(sgp_label, spacegroup)#cls
    #print(sgp_label, spacegroup,bgp_label)
    bgp_l = loss2(bgp_label, bandgaps)#reg
    sp_l = loss2(sp_label, stablility)#reg
    
    l = sgp_l + bgp_l + sp_l
    
    return sgp_l,bgp_l,sp_l, l

def training(train_iter,val_iter, net, devices, num_steps,valstep,anime=False,lr=0.005):
    net = nn.DataParallel(net, device_ids=devices).to(devices[0])
    trainer = torch.optim.Adam(net.parameters(), lr=lr)
    step, timer = 0, d2l.Timer()
    if anime == True:
        animator = d2l.Animator(xlabel='step', ylabel='loss', xlim=[1, num_steps],ylim=[0, 5], legend=['sgp', 'bgp','sp'])
    ...
    while step < num_steps and not num_steps_reached:
        ...
        for element_ids,proportions,valid_lens,spacegroup,bandgaps,stablility in train_iter:
            ...
            trainer.zero_grad()
            timer.start()
            sgp_l,bgp_l,sp_l, l = _get_batch_loss_bert(
                net, element_ids,proportions,valid_lens,spacegroup,bandgaps,stablility)
            l.backward()
            trainer.step()
            metric.add(sgp_l,bgp_l,sp_l, element_ids.shape[0], 1)
            timer.stop()
            if anime == True:
    ...
    print(f'sgp loss {metric[0] / metric[4]:.3f}, '
          f'bgp loss {metric[1] / metric[4]:.3f}, '
...
def validate(val_iter, net, devices):
    ...
    for element_ids,proportions,valid_lens,spacegroup,bandgaps,stablility in val_iter:
        ...
        stablility = stablility.to(devices[0])
        with torch.no_grad():
            sgp_l,bgp_l,sp_l, _ = _get_batch_loss_bert(
                net, element_ids,proportions,valid_lens,spacegroup,bandgaps,stablility)
        metric.add(sgp_l,bgp_l,sp_l, element_ids.shape[0],1)
    # 返回验证损失
